...
(function ($, window, undefined) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					var state = History.getState(),
						bitty_history = '&is_bitty_history=no',
						bittyHistoryReg = new RegExp(bitty_history, 'g');
		...
		bindLink: function() {
			...
			$('body').delegate('a:not([target=_blank],[target=_top],[target=_parent],[target=_self])', 'click', function() {
				var t = $(this),
					...
					isHistory = t.attr('data-is-history'),
					isScrollTop = t.attr('data-is-s-top'),
					options;
	...
	History.Adapter.bind(window, 'statechange', function() {
		var bt = app.bitty,
			...
			url = actualState.url,
			isBittyHistory = url.match(/&bitty_history=yes/) ? true : false;
